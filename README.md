...
- Игра при старте дев-сервера через npm run start иногда не отображает игровые объекты на сцене (sic!). Но сама сборка работает отлично. Исправляется перезагрузкой страницы.

- Иногда приходится использовать sleep(), чтобы сделать Систему асинхронной. Это нужно для того, чтобы получить по событиям нужные Сущности. Это я исправлю переписав механизм уведомления об изменении Компонентов на очереди. При добавлении Компонентов к Сущностям будет формироваться очередь событий, а уже после все события будут иниицированы.
