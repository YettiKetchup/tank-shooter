...
Игра рестартует через location.reload() вместо того, чтобы перезагрузить сцену через StageManager, поскольку тот пока не доделан. Также игра не заканчивается поражением, когда заканчиваются все боеприпасы.
