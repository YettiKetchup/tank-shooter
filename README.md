...
- ⚠️ Полет гранаты по кривой траектории (анимация кривенькая, сейчас фикшу)
...
Игра рестартует через location.reload() вместо того, чтобы перезагрузить сцену через StageManager, поскольку тот пока не доделан. Не реализованы партиклы взрывов (сейчас в прцоессе). Также игра не заканчивается поражением, когда заканчиваются все боеприпасы.
