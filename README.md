...
Ну и сырость интеграционного с Pixi слоя тоже удобства работе не добавляет. Обработка событий на лики кнопки это жесть.
...
Да в целом, **почти** все, что указано в ТЗ, и немного больше.
...
- ⚠️ Полет гранаты по кривой траектории
...
Игра рестартует через location.reload() вместо того, чтобы перезагрузить сцену через StageManager, поскольку тот пока не доделан.Не реализованы партиклы взрывов. Также игра не заканчивается поражением, когда заканчиваются все боеприпасы.
